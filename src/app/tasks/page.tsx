...
const TasksPage = () => {
    ...
    const getStatusColumns = () => {
        return [
            ...
            { id: 'IN_PROGRESS', title: 'In Progress', status: 'IN_PROGRESS', color: 'amber' },
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            case 'WORK_STATUS_REVIEW': return <AlertTriangle className="w-3 h-3" />;
            case 'REBUTTAL': return <AlertTriangle className="w-3 h-3" />;
    ...
    const filteredTasks = tasks.filter(task => {
        ...
        if (filterAssignee !== 'ALL' && !task.assignee.includes(filterAssignee)) return false;
        return true;
    });
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {getStatusColumns().map(column => (
                        <div key={column.id} className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h2 className={`text-lg font-semibold ${getColumnHeaderColor(column.color)}`}>
                                    {column.title}
                                </h2>
