...
const PatientsPage = () => {
    ...
    const getStatusBadge = (status: Patient['workStatus'], overdueDays?: number) => {
        const isOverdue = overdueDays && overdueDays > 30;

        switch (status) {
            case 'TTD':
                return (
                    <Badge variant={isOverdue ? "destructive" : "outline"} className={`gap-1 font-medium ${isOverdue ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200'
                        }`}>
            ...
            case 'MODIFIED':
                return (
                    <Badge variant={isOverdue ? "destructive" : "outline"} className={`gap-1 font-medium ${isOverdue ? 'bg-red-50 text-red-700 border-red-200' : 'bg-amber-50 text-amber-700 border-amber-200'
                        }`}>
                        {isOverdue && <AlertTriangle className="w-3 h-3" />}
            ...
            case 'REGULAR':
                return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 font-medium">Regular</Badge>;
        }
    };
    ...
    const getPriorityBadge = (priority: Patient['priority']) => {
        switch (priority) {
            case 'high':
                return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 font-medium">High</Badge>;
            case 'medium':
                return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 font-medium">Medium</Badge>;
            case 'low':
                return <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200 font-medium">Low</Badge>;
        }
    };
    ...
    const filteredPatients = patients.filter(patient => {
        ...
        const matchesStatus = statusFilter === 'all' || patient.workStatus === statusFilter;
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="p-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-semibold text-gray-900 mb-2">Patients</h1>
                ...
                <Card className="shadow-sm border-0 bg-white mb-8">
                    <CardHeader className="pb-4 border-b border-gray-100">
                        <div className="flex items-center gap-2">
                            ...
                            <CardTitle className="text-xl font-semibold text-gray-900">Search & Filter</CardTitle>
                        </div>
                    </CardHeader>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-semibold text-gray-900">
                                        {patients.filter(p => p.overdueDays && p.overdueDays > 30).length}
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-semibold text-gray-900">
                                        {patients.filter(p => p.lastActivity.includes('day')).length}
                                    </p>
                                </div>
                            </div>
                        </CardContent>
