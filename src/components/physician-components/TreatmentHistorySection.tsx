...
const TreatmentHistory = ({ documentData }) => {
  ...
  const transformData = () => {
    ...
    Object.entries(treatmentHistory).forEach(([systemKey, systemData]) => {
      ...
      if (config && (systemData?.current?.length > 0 || systemData?.archive?.length > 0)) {
        ...
        const archive = (systemData.archive || []).map((entry, index) => ({
          ...
          date: entry.date || 'No date',
          title: entry.event ? `${entry.event} (archive)` : 'Treatment Event (archive)',
          detail: entry.details || 'No details available',
          isArchive: true
        }));
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6 p-6">
      <div className="border-b border-gray-200 pb-3 mb-4">
        <h3 className="text-lg font-semibold text-gray-800">📋 Treatment History by System</h3>
      </div>
      ...
      {systems.length > 0 ? (
        systems.map((system) => (
          <div key={system.id} className="mb-4">
            {/* System Header */}
            <div 
              className={`${systemColors[system.color]} p-4 rounded-lg cursor-pointer transition-all duration-200 hover:opacity-90 flex justify-between items-center font-semibold select-none mt-3`}
                ...
                {system.timeline.map((entry) => (
                  <div 
                    ...
                    className={`mb-3 p-4 bg-gray-50 rounded-lg cursor-pointer border border-gray-200 transition-all duration-200 hover:bg-gray-100 select-none ${
                      expandedTimelineEntries[`${system.id}-${entry.id}`] ? 'bg-gray-100' : ''
                    }`}
                        ...
                        {system.archive.map((archiveEntry) => (
                          <div 
                            ...
                            className={`mb-2 p-3 bg-gray-50 rounded-lg cursor-pointer border border-gray-200 transition-all duration-200 hover:bg-gray-100 select-none ${
                              expandedTimelineEntries[`${system.id}-${archiveEntry.id}`] ? 'bg-gray-100' : ''
                            }`}
                            onClick={() => toggleTimelineEntry(system.id, archiveEntry.id)}
                          >
                            <div className="flex justify-between items-center">
                              <span className="text-sm font-semibold text-gray-900">
                              ...
                              <span className="text-gray-400 font-bold transition-transform duration-200">
                                {expandedTimelineEntries[`${system.id}-${archiveEntry.id}`] ? '▾' : '▸'}
                              </span>
                            </div>
                            {expandedTimelineEntries[`${system.id}-${archiveEntry.id}`] && (
        ...
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-lg font-medium">No treatment history data available</p>
          <p className="text-sm mt-1">Treatment records will appear here when available</p>
        </div>
      )}
    </div>
  );
