...
export default function IntakeModal({ isOpen, onClose, selectedPatient }: IntakeModalProps) {
  ...
  useEffect(() => {
    if (selectedPatient && isOpen) {
      const dobDate = selectedPatient.dob ? new Date(selectedPatient.dob).toISOString().split('T')[0] : '';
      setFormData((prev) => ({
        ...prev,
        lkPatient: selectedPatient.patientName || prev.lkPatient,
        lkDob: dobDate || prev.lkDob,
        lkClaimNumber: selectedPatient.claimNumber && selectedPatient.claimNumber !== "Not specified" 
