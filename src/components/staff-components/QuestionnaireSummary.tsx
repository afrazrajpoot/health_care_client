...
export default function QuestionnaireSummary({
  ...
  return (
    ...
    >
      <h3 className="m-0 px-3.5 py-3 text-base font-bold border-b border-gray-200">Pre‑Visit Questionnaire Summary</h3>
      <div className="flex flex-wrap gap-2 px-4 pb-4 pt-3">
        {chips.map((chip, idx) => (
          <span key={idx} className={`text-xs px-2 py-1 rounded-full border font-semibold whitespace-nowrap ${getChipClasses(chip.type)}`}>
            {chip.text}
          </span>
        ))}
      </div>
